...
export default function BorrowerDetailPage() {
  ...
  const handleAddDebt = async (e) => {
    ...
    if (debtForm.category === "Beverages" && !debtForm.bottleType) {
      setError("Please choose a bottle option for beverage debts.");
      return;
    }
  ...
  const handlePay = async (e) => {
    ...
    setError("");
    try {
      await api.put(`/debts/${payModalOpen._id}/pay`, {
  ...
  if (!borrower)
    return (
      <p className="text-muted">
        Borrower not found.{" "}
        <Link to="/store/borrowers" className="text-ledger-700 underline">
          Go back
        </Link>
      </p>
    );
  ...
  return (
    <div>
      <Link
        to="/store/borrowers"
        className="text-sm text-ledger-700 hover:underline"
      >
        ← All borrowers
      </Link>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 mt-3 mb-6">
        <div>
          ...
          {borrower.contactNumber && (
            <p className="text-sm text-muted mt-1">{borrower.contactNumber}</p>
          )}
        </div>
      ...
      <div className="grid grid-cols-3 gap-2.5 sm:gap-4 mb-6">
        <div className="card p-3 sm:p-5">
          ...
          <p className="font-display text-lg sm:text-2xl text-ink mt-1">
            {peso(borrower.totalDebt)}
          </p>
        </div>
        <div className="card p-3 sm:p-5">
          <p className="text-xs uppercase tracking-wide text-muted font-semibold">
          ...
          <p className="font-display text-lg sm:text-2xl text-ledger-600 mt-1">
            {peso(borrower.totalPaid)}
          </p>
        </div>
        <div className="card p-3 sm:p-5">
          <p className="text-xs uppercase tracking-wide text-muted font-semibold">
          ...
          <p
            className={`font-display text-lg sm:text-2xl mt-1 ${borrower.balance > 0 ? "text-rust" : "text-ledger-600"}`}
          >
            {peso(borrower.balance)}
          </p>
        </div>
      </div>
              ...
              <tbody>
                {debts.map((d) => (
                  <tr key={d._id} className="border-t border-ink/5">
                    ...
                    <td className="px-4 py-3">
                      {peso(d.balance ?? d.totalAmount - d.amountPaid)}
                    </td>
                    <td className="px-4 py-3 text-muted">
                      {fmtDate(d.dateBorrowed)}
                    </td>
                    <td className="px-4 py-3 text-muted">
                      {daysSince(d.dateBorrowed)}d
                    </td>
                    <td className="px-4 py-3">
      ...
      >
        <form onSubmit={handleAddDebt} className="space-y-4">
          ...
          {debtForm.category === "Beverages" && (
            <div>
              <label className="label">Bottle option</label>
              <select
                className="input"
                value={debtForm.bottleType}
                onChange={(e) =>
                  setDebtForm({
                    ...debtForm,
              ...
              >
                ...
                <option value="with_bottle">With Bottle</option>
                <option value="without_bottle">Without Bottle</option>
              </select>
            </div>
          )}
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setDebtForm({ ...debtForm, quantity: e.target.value })
                }
              />
            </div>
            <div>
              ...
              <input
                ...
                className="input"
                required
                value={debtForm.unitPrice}
                onChange={(e) =>
                  setDebtForm({ ...debtForm, unitPrice: e.target.value })
                }
              />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                type="date"
                className="input"
                value={debtForm.dateBorrowed}
                onChange={(e) =>
                  setDebtForm({ ...debtForm, dateBorrowed: e.target.value })
                }
              />
            </div>
            <div>
              ...
              <input
                ...
                min="1"
                className="input"
                value={debtForm.dueInDays}
                onChange={(e) =>
                  setDebtForm({ ...debtForm, dueInDays: e.target.value })
                }
              />
            </div>
          </div>
      ...
      <Modal
        ...
        title={`Pay — ${payModalOpen?.productName || ""}`}
      >
        {payModalOpen && (
          <form onSubmit={handlePay} className="space-y-4">
            ...
            <p className="text-sm text-muted">
              ...
              <span className="font-semibold text-ink">
                {peso(
                  payModalOpen.balance ??
                    payModalOpen.totalAmount - payModalOpen.amountPaid,
                )}
              </span>
            </p>
            <div>
